                    ...
                    _data.delete('checks', checkId, err => {
                      ...
                      if (checksDeleted === checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          cb(500, {
                            error:
                              'Errors occurred while trying to remove user checks data',
                          });
                        }
                      }
                    });
    ...
    if (protocol || url || method || successCodes || timeoutSeconds) {
      ...
      _data.read('checks', id, (err, checkData) => {
        if (!err && checkData) {
          ...
          cb(400, { error: 'Check does not exist' });
        }
      });
    } else {
      cb(400, { error: 'Missing fields to update' });
                ...
                if (!err) {
                  ...
                  _data.read('users', checkData.userPhone, (err, userData) => {
                    if (!err && userData) {
                      ...
                      // remove the check from the user list
                      ...
                      if (checkPosition > -1) {
                        ...
                        cb(500, {
                          error: "Could not find the check on the user's list",
                        });
                      }
                    } else {
                      cb(400, { error: 'Could not find the user' });
                  ...
                  cb(500, { error: 'Could not delete check data' });
                }
              });
            } else {
              cb(403);
